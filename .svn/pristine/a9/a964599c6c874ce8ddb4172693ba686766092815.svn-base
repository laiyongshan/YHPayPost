 ...
 * 　　　　┗┻┛　┗┻┛+ + + +
 * <p>
 * ClassName：Level1Fragment
 * Description: 黄金等级界面
 * Author：JensenWei
 * QQ: 2188307188
 * Createtime：2017/5/18 11:57
 * Modified By：
 * Fixtime：2017/5/18 11:57
 * FixDescription：
 */
    ...
    public void setAData() {
        if (feeListBean.size() > 0) {
            for (GetMerchLevelResults.DataBean.FeeListBean bean : feeListBean) {
                if (bean.getMerchLevel().equals("1") && bean.getServiceId().equals("04"))//微信
                {
                ...
                if (bean.getMerchLevel().equals("1") && bean.getServiceId().equals("02"))//境内快捷
                {
                ...
                if (bean.getMerchLevel().equals("1") && bean.getServiceId().equals("03"))//境外快捷
                {
