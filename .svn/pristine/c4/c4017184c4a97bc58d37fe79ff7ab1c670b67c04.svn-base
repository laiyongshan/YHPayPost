...
public class WithDrawActivity extends XActivity<PWithDraw> {
    ...
    public void initData(Bundle savedInstanceState) {
        dataBean = (GetAccountResults) getIntent().getSerializableExtra("dataBean");
        initAnim();
        initView();
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.withdraw_tv:
                ...
                if (dataBean.getType().equals("0")) {
                    getP().withDraw(AppConfig.TIXIAN, String.valueOf(AppKit.subString(dataBean.getAvlAmt(), AppUser.getInstance().getFee())));
                } else {
                    getP().withDraw(AppConfig.SYTIXIAN, String.valueOf(AppKit.subString(dataBean.getAvlAmt(), AppUser.getInstance().getFee())));
                }
                break;
            case R.id.withdraw_confirm_bt:
                getvDelegate().showLoading();
                if (dataBean.getType().equals("0")) {
                    getP().withDraw(AppConfig.TIXIAN, withdrawAmtEt.getText().toString());
                } else {
                    getP().withDraw(AppConfig.SYTIXIAN, withdrawAmtEt.getText().toString());
                }
                break;
        }
    }
    ...
    public void setSettleInfo(GetMerchInfoResult.DataBean dataBean) {
        ...
        if (acctNo != null && acctNo.length() > 4) {
            withdrawSettleTv.setText(dataBean.getBankName() + "  (" + acctNo.substring(acctNo.length() - 4, acctNo.length()) + ")");
        }
    }
